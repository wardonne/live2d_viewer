...
LAppLive2DManager.prototype.loadLive2d = function (gl, model, callback) {
    ...
    if (this.reloadFlg) {
        ...
        this.createModel();
        this.models[0].load(gl, model, callback);
    }
};
...
LAppLive2DManager.prototype.tapEvent = function (x, y) {
    ...
    for (var i = 0; i < this.models.length; i++) {
        ...
        if (this.models[i].hitTest(LAppDefine.HIT_AREA_HEAD, x, y)) {
            ...
            if (LAppDefine.DEBUG_LOG)
                console.log("Tap head." + " models[" + i + "]");

            this.models[i].startRandomMotion(LAppDefine.MOTION_GROUP_TAP_HEAD,
                LAppDefine.PRIORITY_NORMAL);
            ...
            this.models[i].startRandomMotion(LAppDefine.MOTION_GROUP_TAP_BODY,
                LAppDefine.PRIORITY_NORMAL);
        } else if (this.models[i].hitTest(LAppDefine.HIT_AREA_LEG, x, y)) {

            if (LAppDefine.DEBUG_LOG)
                console.log("Tap leg." + " models[" + i + "]");

            this.models[i].startRandomMotion(LAppDefine.MOTION_GROUP_TAP_LEG,
                LAppDefine.PRIORITY_NORMAL);
