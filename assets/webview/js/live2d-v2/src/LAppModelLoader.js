...
function init() {
    ...
    if (!this.gl) {
        Live2dErr("Failed to create WebGL context.");
        return;
    }
...
function loadLive2d(model, callback) {
    if (!model) {
        return;
    }
    this.isModelShown = false;
    ...
    this.live2DMgr.loadLive2d(this.gl, model, callback);
...
function followPointer(event) {
    ...
    if (thisRef.drag) {
        console.log(123);
        var modelMatrix = window.live2DMgr.getModel(0).modelMatrix
        modelMatrix.setCenterPosition(sx, sy);
        // thisRef.lastMouseX = sx;
        // thisRef.lastMouseY = sy;
        // thisRef.dragMgr.setPoint(vx, vy);
    }
...
function mouseEvent(e) {
    ...
    if (e.type == "mousewheel") {
        ...
        thisRef.canvas.addEventListener('mousemove', move)
        var mouseup = function () {
            thisRef.canvas.removeEventListener('mousemove', move);
            thisRef.canvas.removeEventListener('mouseup', mouseup);
        };
        thisRef.canvas.addEventListener('mouseup', mouseup);
...
function Live2dErr(msg) {
    if (!LAppDefine.DEBUG_LOG) return;
    console.error("<span style='color:red'>" + msg + "</span>");
};